...
Built by Canton Foundation Developer Relations for hackathons, bootcamps, and anyone who needs a local Canton Network without the 10-day DevNet whitelisting wait, jFrog logins, or Makefile archaeology.

```bash
canton devrel start
```

That's it. Three validators, a synchronizer, wallet UIs, Canton Coin, Keycloak, PQS — the whole stack.
...
The installer handles everything else: downloads the tool, sets up PATH, and optionally fixes `/etc/hosts` for `*.localhost` domains.
...
canton devrel deploy ./my-app-0.0.1.dar    # upload your DAR
canton devrel logs                         # tail all logs
canton devrel logs splice                  # tail one service
...
| PostgreSQL | localhost:5432 | cnadmin / supersafe |
...
**Is:** A stripped-down Canton LocalNet — pure network infrastructure. Three validators, a synchronizer, wallets, Keycloak, PQS. No reference app noise.

**Isn't:** A replacement for [cn-quickstart](https://github.com/digital-asset/cn-quickstart). This tool wraps the same Splice LocalNet images but removes the build step, the Makefile, and all the reference app scaffolding so you can focus on your own project.

**Not for production.** This is a local development environment. Admin ports and credentials are intentionally exposed for developer convenience.
...
Canton Foundation Developer Relations.
For the LocalNet guide, tooling catalogue, and more: [Canton Developer Hub](https://github.com/canton-foundation/canton-devs-guide).
